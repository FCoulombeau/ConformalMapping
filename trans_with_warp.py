...
@version : 0.3
@change :
(0.3)   Use of skimage.transform.warp to actually do the transformation.
...
class ImageTransform:
    ...
    def _f(self,z):
        ...
        if self._transformations:
            ...
            return u
    def _f1(self,xy):
        # For unknown reasons, skimage.transform.warp introduces a slight
        # zooming that should be corrected.
        # Yet, the correction factor appears to be random...
        xy=xy*2./self._height
        u=(xy[:,0]-1j*xy[:,1]-self._output_scaling+1j+0.0001+self.d)/self.c       
        if self._transformations:
            z=_compose(*self._transformations)(u)
            return np.floor([[k.real,-k.imag] for k in self._reflect*(z+self._input_scaling+1j)*self.data.shape[0]/2.])
        else:
            return np.floor([[k.real,-k.imag] for k in self._reflect*(u+self._input_scaling+1j)*self.data.shape[0]/2.])
    ...
    def _findColors(self,infini=True,liste=[[0,0]],MaxX=np.NaN,color=[255]*3,mode='wrap'):
        FPt=[]        
        res=skt.warp(self.data,self._f1,mode=mode,output_shape=(self._height,self._width),order=1)
        if not(np.isnan(MaxX)):
            bx,by,ex,ey=self._output_rect[0].real,self._output_rect[0].imag,self._output_rect[1].real,self._output_rect[1].imag
            grid=np.mgrid[by:ey:(self._height*1j),bx:ex:(self._width*1j)]
            grid=(grid[0]*1j+grid[1]).conjugate()
    ...
    def transform(self,infinite=True,lst=[[0,0]],MaxX=np.NaN,color=[255]*3,print_and_save=True,mode='wrap'):
        """Perform the transformation(s).
        If infinite=True, the whole plane is covered by the input image or half
        the plane if MaxX is given.
        If infinite=False, the input is placed on places given in lst.
        ...
        if mode=='reflect':
            ...
            self._reflect=1
        Couleurs=self._findColors(infini=infinite,liste=lst,color=color,MaxX=MaxX,mode=mode)
